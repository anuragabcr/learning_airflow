...
CREATE TABLE IF NOT EXISTS `learning-0301.silver_dataset.departments` (
    Dept_Id STRING,
    SRC_Dept_Id STRING,
    Name STRING,
    datasource STRING,
...
TRUNCATE TABLE `learning-0301.silver_dataset.departments`;

-- 3. full load by Inserting merged Data 
INSERT INTO `learning-0301.silver_dataset.departments`
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `learning-0301.bronze_dataset.departments_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `learning-0301.bronze_dataset.departments_hb`
);

-------------------------------------------------------------------------------------------------------

-- 1. Create table providers by Merge Data from Hospital A & B  
CREATE TABLE IF NOT EXISTS `learning-0301.silver_dataset.providers` (
    ProviderID STRING,
    FirstName STRING,
    LastName STRING,
...
-- 2. Truncate Silver Table Before Inserting 
TRUNCATE TABLE `learning-0301.silver_dataset.providers`;

-- 3. full load by Inserting merged Data 
INSERT INTO `learning-0301.silver_dataset.providers`
SELECT DISTINCT 
    ProviderID,
    FirstName,
    LastName,
    ...
    END AS is_quarantined
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `learning-0301.bronze_dataset.providers_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `learning-0301.bronze_dataset.providers_hb`
);

-------------------------------------------------------------------------------------------------------

-- 1. Create patients Table in BigQuery
CREATE TABLE IF NOT EXISTS `learning-0301.silver_dataset.patients` (
    Patient_Key STRING,
...
CREATE OR REPLACE TABLE `learning-0301.silver_dataset.quality_checks` AS
SELECT DISTINCT 
    CONCAT(CAST(SRC_PatientID AS STRING), '-', datasource) AS Patient_Key,
    SRC_PatientID,
    FirstName,
    LastName,
    MiddleName,
...
FROM (
    SELECT DISTINCT 
        ...
        Gender,
        CASE
          WHEN ABS(DOB) > 100000000000
            THEN DATE(TIMESTAMP_MILLIS(DOB))
          WHEN LENGTH(CAST(DOB AS STRING)) = 8
            THEN PARSE_DATE('%Y%m%d', CAST(DOB AS STRING))
          ELSE NULL
        END AS DOB,
        Address,
        DATE(TIMESTAMP_MILLIS(ModifiedDate)) AS ModifiedDate,

        'hosa' AS datasource
    FROM `learning-0301.bronze_dataset.patients_ha`
    UNION ALL
    SELECT DISTINCT 
        ID AS SRC_PatientID,
        F_Name AS FirstName,
        ...
        'hosb' AS datasource
    FROM `learning-0301.bronze_dataset.patients_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `learning-0301.silver_dataset.patients` AS target
USING `learning-0301.silver_dataset.quality_checks` AS source
...
WHEN MATCHED AND (
    ...
    DATE(TIMESTAMP_MILLIS(target.DOB)) <> source.DOB OR
    target.Address <> source.Address OR
    DATE(TIMESTAMP_MILLIS(target.SRC_ModifiedDate)) <> source.SRC_ModifiedDate OR
...
VALUES (
    ...
    source.PhoneNumber,
    source.Gender,
    UNIX_MILLIS(TIMESTAMP(source.DOB)),
    source.Address,
    UNIX_MILLIS(TIMESTAMP(source.SRC_ModifiedDate)),
    source.datasource,
...
DROP TABLE IF EXISTS `learning-0301.silver_dataset.quality_checks`;

-------------------------------------------------------------------------------------------------------

-- 1. Create transactions Table in BigQuery
CREATE TABLE IF NOT EXISTS `learning-0301.silver_dataset.transactions` (
...
SELECT DISTINCT 
    ...
    END AS is_quarantined
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `learning-0301.bronze_dataset.transactions_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `learning-0301.bronze_dataset.transactions_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `learning-0301.silver_dataset.transactions` AS target
USING `learning-0301.silver_dataset.quality_checks` AS source
...
CREATE TABLE IF NOT EXISTS `learning-0301.silver_dataset.encounters` (
...
CREATE OR REPLACE TABLE `learning-0301.silver_dataset.quality_checks_encounters` AS
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `learning-0301.bronze_dataset.encounters_ha`
    ...
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosb' AS datasource
    FROM `learning-0301.bronze_dataset.encounters_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `learning-0301.silver_dataset.encounters` AS target
USING `learning-0301.silver_dataset.quality_checks_encounters` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `learning-0301.silver_dataset.quality_checks_encounters`;

-------------------------------------------------------------------------------------------------------

-- 1. Create the Claims Table in BigQuery
CREATE TABLE IF NOT EXISTS `learning-0301.silver_dataset.claims` (
    Claim_Key STRING,
    SRC_ClaimID STRING,
...
CREATE OR REPLACE TABLE `learning-0301.silver_dataset.quality_checks_claims` AS
SELECT 
...
FROM (
    SELECT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `learning-0301.bronze_dataset.claims`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `learning-0301.silver_dataset.claims` AS target
USING `learning-0301.silver_dataset.quality_checks_claims` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `learning-0301.silver_dataset.quality_checks_claims`;

-------------------------------------------------------------------------------------------------------

-- 1. Create the CP Codes Silver Table in BigQuery
CREATE TABLE IF NOT EXISTS `learning-0301.silver_dataset.cpt_codes` (
    CP_Code_Key STRING,
...
CREATE OR REPLACE TABLE `learning-0301.silver_dataset.quality_checks_cpt_codes` AS
SELECT 
...
FROM (
    SELECT 
        ...
        code_status,
        'hosa' AS datasource
    FROM `learning-0301.bronze_dataset.cpt_codes`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `learning-0301.silver_dataset.cpt_codes` AS target
USING `learning-0301.silver_dataset.quality_checks_cpt_codes` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `learning-0301.silver_dataset.quality_checks_cpt_codes`;
